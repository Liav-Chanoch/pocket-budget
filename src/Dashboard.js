...
import { useOfflineToast, notifyOffline } from './offlineToast';
import { queuedWrite, newDocRef } from './firestoreWrite';
...
function ShoppingListTab({ groupId, user, currency, groupCurrency, country: initialCountry, onCountryChange, estimation, onEstimationChange, estimationOpen, onEstimationOpenChange, t, onOpenOtherLists }) {
  ...
  useEffect(() => {
    getDocs(collection(db, 'groups', groupId, 'product_catalog', user.uid, 'items'))
      ...
      .catch(() => {});   // offline with a cold cache: fall back to an empty catalog
  }, [groupId, user.uid]);
  ...
  async function addItemWithPrice() {
    ...
    if (!isNaN(amt) && amt > 0) {
      ...
      if (existing) {
        await queuedWrite(updateDoc(doc(catRef, existing.id), catData));
        setCatalog(prev => prev.map(c => c.id === existing.id ? { ...c, ...catData } : c));
      } else {
        const newRef = newDocRef(catRef);
        await queuedWrite(setDoc(newRef, { name: trimmed, ...catData }));
        setCatalog(prev => [...prev, { id: newRef.id, name: trimmed, ...catData }]);
      }
    }
  ...
  async function handleJustDelete() {
    setEstimation(null);
    await queuedWrite(deleteDoc(doc(db, 'groups', groupId, 'shopping_list', buyItem.id)));
    setBuyItem(null);
  }
  ...
  async function handleAddAsExpense() {
    ...
    setBuying(true);
    await queuedWrite(addDoc(collection(db, 'groups', groupId, 'expenses'), {
      uid: user.uid, addedBy: user.displayName || user.email,
    ...
    if (existing) {
      ...
      const newRef = newDocRef(catRef);
      await queuedWrite(setDoc(newRef, { name: buyItem.text, ...catData }));
      setCatalog(prev => [...prev, { id: newRef.id, name: buyItem.text, ...catData }]);
    }
    setEstimation(null);
    await queuedWrite(deleteDoc(doc(db, 'groups', groupId, 'shopping_list', buyItem.id)));
    setBuyItem(null);
  ...
  async function runEstimation() {
    // Still useful offline: personal catalog and the local price DB resolve
    // without network. Only the AI fallback is unavailable.
    if (!navigator.onLine) notifyOffline(t.offlineEstimates);
    setEstimating(true);
    setEstimation(null);
  ...
  async function handleFindNearbyShops(targetItems) {
    ...
    if (!navigator.onLine) {
      notifyOffline(t.offlineNearbyShops);
      return;
    }
...
function OtherListDetail({ listId, listName, listEmoji, groupId, user, currency, groupCurrency, t, lang, onBack }) {
  ...
  async function addItemWithPrice() {
    ...
    if (!isNaN(amt) && amt > 0) {
      ...
      const catRef = collection(db, 'groups', groupId, 'product_catalog', user.uid, 'items');
      const catData = { price: amt, originalPrice: amt, originalCurrency: groupCurrency, category: newCategory };
      if (existing) { await queuedWrite(updateDoc(doc(catRef, existing.id), catData)); setCatalog(prev => prev.map(c => c.id === existing.id ? { ...c, ...catData } : c)); }
      else { const nr = newDocRef(catRef); await queuedWrite(setDoc(nr, { name: trimmed, ...catData })); setCatalog(prev => [...prev, { id: nr.id, name: trimmed, ...catData }]); }
    }
    await addItem(trimmed);
  }
  ...
  async function handleJustDelete() {
    setEstimation(null);
    await queuedWrite(deleteDoc(listDoc(buyItem.id)));
    setBuyItem(null);
  }
  ...
  async function handleAddAsExpense() {
    ...
    const catData = { price: unitAmt, originalPrice: unitAmt, originalCurrency: groupCurrency, category: buyCategory };
    if (existing) { await queuedWrite(updateDoc(doc(catRef, existing.id), catData)); setCatalog(prev => prev.map(c => c.id === existing.id ? { ...c, ...catData } : c)); }
    else { const nr = newDocRef(catRef); await queuedWrite(setDoc(nr, { name: buyItem.text, ...catData })); setCatalog(prev => [...prev, { id: nr.id, name: buyItem.text, ...catData }]); }
    setEstimation(null);
    await queuedWrite(deleteDoc(listDoc(buyItem.id)));
...
export default function Dashboard({ user, groupId, group, memberData, onLogout }) {
  ...
  const offlineToast = useOfflineToast();
  ...
  useEffect(() => {
    ...
    async function processDays() {
      ...
      if (!lastProcessed) {
        await queuedWrite(updateDoc(doc(db, 'groups', groupId, 'members', user.uid), { last_day_processed: yesterday }));
        return;
      }
      ...
      while (processDate <= yesterday) {
        ...
        runningBalance += net;
        await queuedWrite(setDoc(doc(db, 'groups', groupId, 'daily_records', `${user.uid}_${processDate}`), {
      ...
      await queuedWrite(updateDoc(doc(db, 'groups', groupId, 'members', user.uid), {
  ...
  async function addExpense(e) {
    ...
    await queuedWrite(addDoc(collection(db, 'groups', groupId, 'expenses'), expDoc));
    ...
    if (form.description.trim()) {
      ...
      if (existing) {
        await queuedWrite(updateDoc(doc(catRef, existing.id), catalogUpdate));
      } else {
        await queuedWrite(addDoc(catRef, { name, ...catalogUpdate }));
      }
    }
  ...
  async function deleteExpense(id) {
    ...
    await queuedWrite(deleteDoc(doc(db, 'groups', groupId, 'expenses', id)));
  ...
  async function processScanFile(file) {
    if (!file) return;
    if (!navigator.onLine) {
      notifyOffline(t.offlineScanReceipt);
      return;
    }
    setIsScanning(true);
    try {
    ...
    <>
      ...
      {offlineToast && (
        <div className="offline-toast">
          <WifiOff size={16} strokeWidth={2} className="offline-toast-icon" />
